...
void Rms_Task(void)
{
	...
	{	
		...
		if(adjustState == 0 && run_count == 5)		//500ms读取一次输出电压
		{
...
void Protection_Task(void)
{
	...
	if(power_p == 4 && AVR_Handle.electric < MAX_ELE && AVR_Handle.ele_protect < 3)
	{
		warning_twink = 1;
		power_p = 0;
	}
	...
	else
	{
		...
		else
		{
			...
			if(time_count2 >= 300)
			{
				...
				AIP1820_printf(SEG5,0x01);						//显示P-H
				AIP1820_printf(SEG6,0x73);
			}			
		}
		if(time_count2 <= 300)
		{
